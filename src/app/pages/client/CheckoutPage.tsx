...
export default function CheckoutPage() {
  ...
  if (success) {
    return (
      <div className="container mx-auto px-4 py-24 text-center max-w-lg min-h-[70vh] flex flex-col items-center justify-center">
        <div className="w-24 h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-8 shadow-inner">
          <Check size={48} strokeWidth={3} />
        </div>
        <h2 className="text-4xl font-black mb-4 text-[#1C1A17]">{t('orderSuccessTitle') || 'تم الطلب بنجاح!'}</h2>
        <p className="text-gray-500 mb-8 leading-relaxed text-lg">
          {t('orderSuccessDesc') || 'شكراً لك. سنقوم بالتواصل معك قريباً لتأكيد الطلب وتجهيزه لك بكل حب.'}
        </p>
        <Button onClick={() => navigate('/')} className="bg-[#111111] hover:bg-[#C5A880] text-white px-10 h-14 rounded-full font-bold text-lg transition-colors">
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-20 min-h-screen">
      ...
      <CheckoutForm cart={cartData} onSuccess={() => setSuccess(true)} />
    </div>
  );
}
