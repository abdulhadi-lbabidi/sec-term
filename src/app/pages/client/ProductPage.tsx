...
import { useProduct } from '@/app/api/client/useProducts';
import { toast } from 'sonner';

import { Product, Package } from '@/types/Client/product';
...
export default function ProductPage() {
  ...
  const { language, addToCart, wishlist, toggleWishlist, user } = useAppStore();
  const t = translations[language];
  const [qty, setQty] = useState(1);
  const [reviews, setReviews] = useState<Review[]>([]);

  const { data, isLoading } = useProduct(id!);
  const product: Product | undefined = data as Product;
