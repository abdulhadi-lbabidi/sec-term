    ...
    <div className="rounded-2xl border border-black/10 bg-white p-6 text-black">
      <div className="flex items-center gap-4 border-b border-black/5 pb-4">
        ...
        <h1 className="text-xl font-bold">
          {isEditMode
            ? isRtl ? 'تعديل المتغير' : 'Edit Product Variant'
            : isRtl ? 'إضافة متغيرات المنتج' : 'Add Product Variant'}
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-6">
        ...
        <div className="w-full">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              maxLength={12}
              {...register('barcode', { 
                required: isRtl ? 'الباركود مطلوب' : 'Barcode is required',
                pattern: {
                  value: /^\d{12}$/,
                  message: isRtl ? 'الباركود يجب أن يتكون من 12 رقماً حصراً' : 'Barcode must be exactly 12 digits'
                }
              })}
        ...
        <div className="space-y-3">
          <Label className="text-sm font-semibold">
            {isRtl ? 'الإضافة للباقات' : 'Add to Packages'}
          </Label>
        ...
        <div className="space-y-2">
          <Label className="text-sm font-semibold">
            {isRtl ? 'صورة المتغير' : 'Variant Image'}
          </Label>
        ...
        <div className="flex justify-end gap-3 border-t border-black/5 pt-4">
          ...
          <Button type="submit" disabled={isPending}>
            {isPending ? (
              ...
              isRtl ? 'إضافة المتغير' : 'Add Variant'
            )}
          </Button>
        </div>
      </form>
