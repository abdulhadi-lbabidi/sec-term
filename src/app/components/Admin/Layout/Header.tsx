            ...
            <div className="flex items-start gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-sm font-black tracking-tight text-black sm:text-base md:text-lg leading-tight">
                  ...
                  {isProductsPage && t('admin.manage_products')}
                  {isAddProductPage && (location.pathname.includes('/edit/') ? t('admin.editProduct') : t('admin.addNewProduct'))}
                  {isAddVariantPage && (location.pathname.includes('/edit-variant/') ? t('admin.edit_variant') : t('admin.add_product_variant'))}
                  {isSizesPage && t('admin.manage_sizes')}
                  {isMaterialsPage && t('admin.manage_materials')}
                  {isPackagesPage && t('admin.manage_packages')}
                  {isReviewsPage && t('admin.manage_reviews')}
                  {isRolesPage && t('admin.roles')}
                  {isAddRolePage && t('admin.add_new_role')}
                  {isEditRolePage && t('admin.edit_role')}
                  {isUsersPage && t('admin.users')}
                  {isOrdersPage && t('admin.orders_management')}
                  {isCheckoutsPage && t('admin.checkouts_management')}
                </h1>
                <p className="text-[10px] text-black/50 sm:text-xs mt-0.5 leading-normal max-w-[200px] sm:max-w-xs md:max-w-md">
                  ...
                  {isProductsPage && t('admin.products_description')}
                  {isAddProductPage && t('admin.add_product_description')}
                  {isAddVariantPage && t('admin.add_variant_description')}
                  ...
                  {isReviewsPage && t('admin.reviews_description')}
                  {isRolesPage && t('admin.roles_description')}
                  {isAddRolePage && t('admin.add_role_description')}
                  {isEditRolePage && t('admin.edit_role_description')}
                  {isUsersPage && t('admin.users_description')}
                  {isOrdersPage && t('admin.orders_description')}
                  {isCheckoutsPage && t('admin.checkouts_description')}
                </p>
              </div>
            </div>
